...
import com.gestankbratwurst.ferocore.modules.rolemodule.skills.ShamanGhostBall;
 ...
 *
 */
public class ShamanStaffItemHandle implements CustomItemHandle {
  ...
  public void handleInteracting(final PlayerInteractEvent event, final ItemStack item) {
    ...
    if (!feroPlayer.hasChosenRole() || feroPlayer.getChosenRoleTye() != RoleType.SHAMAN) {
      return;
    }
    if (feroPlayer.getRoleLevel() < 20) {
      return;
    }
    final long cd = SkillModule.getCooldownLeft(player, "SHAMAN_STAFF");
    if (cd > 0) {
      return;
    }
    SkillModule.setCooldown(player, "SHAMAN_STAFF", 3000);
    new ShamanGhostBall(player.getEyeLocation(), feroPlayer).start();
  }
